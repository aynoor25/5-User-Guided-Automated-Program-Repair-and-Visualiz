...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('server_llvm', function (msg) {
      console.log("Message from backend: " + msg + " " + socket.id);
    ...
    socket.on('run', function (try_suggested_fixes) {
      if (backend_socket.connected) {
        clients.forEach(function (client, index) {
          ...
          if (socket == client.socket) {
            ...
            id_client = find_client_by_id(socket);
            console.log("Asked backend server for repair");
            if (id_client > 0) backend_socket.emit('send_data', id_client, try_suggested_fixes);
          }
        });
      } else {
    ...
    socket.on('get_tarantula_results', function () {
      if (backend_socket.connected) {
        clients.forEach(function (client, index) {
          //console.log(client.socket.id + " ============== " + socket.id);
          if (socket == client.socket) {
            ...
            id_client = find_client_by_id(socket);
            if (id_client > 0) {
              backend_socket.emit('send_tarantula_results', id_client);
              console.log("Asked backend server for tarantula results.")
            }
          }
        });
      } else {
    ...
    socket.on('default_repair_process', function () {
      if (backend_socket.connected) {
        clients.forEach(function (client, index) {
          if (socket == client.socket) {
            request_client_socket = client.socket;
            id_client = find_client_by_id(socket);
            if (id_client > 0) backend_socket.emit('default_repair_process', id_client);
          }
        });
      } else {
        console.log("Connection with backend server lost.");
    ...
    socket.on('send_variables', function(variables) {
      console.log(request_client_socket.id);
      console.log("Suggested variables received: " + variables);
      request_client_socket.emit('display_fixes_options', variables);
    });

    socket.on('send_opcodes', function(opcodes) {
      console.log(request_client_socket.id);
      console.log("Suggested opcodes received: " + opcodes);
      request_client_socket.emit('display_fixes_options', opcodes);
    });

    socket.on("client_msg", function(client_msg) {
      request_client_socket = socket;
    ...
    socket.on('Line_Colors', function (line_colors) {
      ...
      //console.log(request_client_socket.id);
      request_client_socket.emit('line_colors', line_colors);
    });

    socket.on('reset_tc', function () {
      //console.log("reset_tc");
      backend_socket.emit('remove_tc_trace');
    });

    socket.on('send_faulty_line_number', function(faulty_line_num) {
      backend_socket.emit('set_faulty_line_number', faulty_line_num);
      request_client_socket = socket;
    });

    socket.on('is_user_guided', function() {
      backend_socket.emit('is_user_guided', "true");
    });

    socket.on('tc_cov_result', function(tc_result) {
      console.log(tc_result);
      request_client_socket.emit('tc_cov_result', tc_result);
    });

    socket.on('show_coverage', function (testcase_name) {
      //console.log(testcase_name);
      //console.log(request_client_socket.id);
      if (backend_socket.connected) {
        clients.forEach(function (client, index) {
          //console.log(client.socket.id + " ============== " + socket.id);
          if (socket == client.socket) {
            request_client_socket = client.socket;
            id_client = find_client_by_id(socket);
            if (id_client > 0) backend_socket.emit('send_converage', id_client, testcase_name);
          }
        });
      } else {
        console.log("Connection with backend server lost.");
