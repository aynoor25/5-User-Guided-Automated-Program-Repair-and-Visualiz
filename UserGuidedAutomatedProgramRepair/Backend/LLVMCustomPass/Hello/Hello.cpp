...
bool try_variable(string var_name) {
  string is_user_guided = read_file("is_user_guided.txt");
  string file_path;
  if (is_user_guided == "true") {
    file_path = get_file_path()+"try_variables.txt";
  } else {
    file_path = get_file_path()+"get_variables.txt";
  }
  ifstream get_variable_names(file_path);
  string line;
  while (getline(get_variable_names, line)) {
    if (line.compare(var_name) == 0) {
      return true;
    }
  }
  return false;
  ...
  struct Replace : public FunctionPass {
    ...
    bool runOnFunction(Function &F) override {
      ...
      if (F.getName() != "main") {
        ...
        // cout <<  try_variable((string) values_to_try[value_index]->getName()) << endl;
        // if (value_index < values_to_try.size() && try_variable((string) values_to_try[value_index]->getName()) == true) {
          //cout << "hi"<<endl;
        if (value_index < values_to_try.size()) {
          string var_name = (string) values_to_try[value_index]->getName();        
          if (try_variable(var_name)) {
            //errs() << var_name << "\n";
          bool replaced = false;
          for (auto &B : F) {
            for (auto &I : B) {
              if (DILocation *Loc = I.getDebugLoc()) { // Here I is an LLVM instruction
                ...
                if (File.find("TC") != string::npos) {}
                  ...
                  if (faulty_line_num == Line) {
                    ...
                    //cout << "try variable: "+  try_variable((string) values_to_try[value_index]->getName()) << endl;
...
namespace {
  struct GetVariables : public FunctionPass {
    static char ID;
    GetVariables() : FunctionPass(ID) {}
    Value* replace_with = NULL;

    bool runOnFunction(Function &F) override {
      ValueSymbolTable& sym_table = F.getValueSymbolTable();
      vector<string> values_to_try;
      for (auto iter1 = sym_table.begin(); iter1 != sym_table.end(); ++iter1) {
        StringMapEntry<Value*> &map_entry = *iter1;
        if (F.getName() != "main") {
          AllocaInst* AI = dyn_cast<AllocaInst>((map_entry.getValue()));
          if (AI) {
            if ((AI->getAllocatedType()->getTypeID()) == 11) {
              //errs() << "Nme " << map_entry.getValue() << "\n";
              values_to_try.push_back((string )map_entry.getValue()->getName());
              // string variable_name = (string )map_entry.getValue()->getName() + "\n";
              // write_variable_names(get_file_path()+"get_vriables.txt", variable_name.c_str());
            }
          }
        }
      }
       ofstream file_name;
        file_name.open("get_variables.txt");
        for (int i = 0; i < values_to_try.size(); ++i) {
          file_name << values_to_try[i] + "\n";
        }
        file_name.close();
      }
  };
...
static RegisterPass<GetVariables> T("get_variables", "For VR fix get replacement variables from the value symbol table of the function.", false, false);
