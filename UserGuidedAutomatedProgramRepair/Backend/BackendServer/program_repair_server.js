...
var socket = io.connect('http://172.16.191.1:8000', {reconnect: true});
...
socket.on('set_faulty_line_number', function (faulty_line_num) {
	fs.writeFile('faulty_line_num.txt', faulty_line_num, function (error) {
    	if (error)  console.log('Faulty line number file could not be saved, error: ' + error);
    	else {
    		console.log("User selected faulty line received.");
    		child = exec("g++ -std=c++0x inst_type.cpp -o it", function (error, stdout, stderr) {			
				if (error !== null) console.log('exec error: ' + error);

				var exe = exec("./it", function (err, out, derr) {
					if (err !== null) console.log('exec err: ' + err);
					// send line colors 
					fs.readFile('fix_type.txt', 'utf8', function (error, fault_type) {
						if (error) console.log(error);
						if (fault_type == "vr") {
							var variables = [];
							lineReader.eachLine('get_variables.txt', function(line, last_line) {
								variables.push(line);
								if (last_line) {
									socket.emit('send_variables', variables);
								}
							});
						} else if (fault_type == "oc") {
							opcodes = ["+", "-", "*", "/"];
							socket.emit('send_opcodes', opcodes);
						}
					});
				});
			});
...
socket.on('is_user_guided', function() {
	fs.writeFile('is_user_guided.txt', "true", function(error) {
    	if (error)  console.log('is_user_guided file could not be saved, error: ' + error);
    	else {console.log("User guided option received received.")}
    });
});


function run_repair(client_id, try_suggested_fixes) {
	console.log("About to run repair");
	fs.writeFile('fixes_to_try.txt', try_suggested_fixes, function(error) {
    	if (error)  console.log('fixes_to_try file could not be saved, error: ' + error);
    	else {console.log("User selected variables received.")}
...
socket.on('send_data', function (client_id, try_suggested_fixes) {
	run_repair(client_id, try_suggested_fixes);
});

function run_fault_localiztion(client_id) {
	child = exec("g++ -std=c++0x  Run_Fault_Localization.cpp -o rfl", function (error, stdout, stderr) {
		var source_code_path = find_client_file_path(client_id, "source");
		var testcase_file_path = find_client_file_path(client_id, "testcase");
		console.log("client id: " + client_id);
		console.log("source code path: "  + source_code_path);
		...
		var exe = exec("./rfl " + source_code_path + " " + testcase_file_path, function (err, out, derr) {
			if (err !== null) console.log('exec err: ' + err);
			// send line colors 
			var line_colors = {};
			var splitted_line;
...
socket.on('send_tarantula_results', function (client_id) {
	run_fault_localiztion(client_id);
});

var Promise = require('promise');

socket.on('default_repair_process', function (client_id) {
	run_fault_localiztion(client_id, run_repair(client_id));	
});
